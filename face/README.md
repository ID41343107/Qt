...
基於 Qt 的簡化門禁介面（已移除 OpenCV 相依性，僅保留基本介面與名單紀錄）

## 系統需求

- Qt 6.x 或更高版本
- 不需 OpenCV（攝影機預覽與人臉功能已停用）
- SQLite3（Qt 已內建）
- C++17 編譯器
- （可選）攝影機（目前預覽功能已停用）

> 注意：此版本已移除 OpenCV 與深度學習模型，相關安裝/下載步驟可略過，介面僅示範資料庫增刪與狀態顯示。
> 升級提醒：舊版如已建立含人臉特徵欄位的 users.db，本版僅使用姓名欄位，建議升級前先備份。
> 說明以中文為主，保留必要專有名詞的英文。
...
> 目前版本關閉人臉辨識與攝影機預覽，僅保留姓名註冊/刪除資料庫與靜態狀態提示。

### 1. 即時攝影機預覽
- 本版本未啟用攝影機預覽（OpenCV 已移除）

### 2. 人臉註冊
1. 在「註冊姓名」欄位輸入使用者姓名
2. 點擊「註冊使用者」按鈕，姓名會寫入 SQLite 名單
3. （人臉擷取功能已停用）

### 3. 人臉辨識與視覺回饋
- 已停用（改以文字狀態顯示「OpenCV removed」）
